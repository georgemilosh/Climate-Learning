...
fig.tight_layout()
fig.savefig(f"Images/mapping{sys.argv[-1]}.png", bbox_inches='tight', dpi=200)
logger.info(f" saved to Images/mapping{sys.argv[-1]}.png")
...
def vae_generate_images(vae,Z_DIM,n_to_show=10):
    ...
    for i in range(n_to_show):
        ...
        if iterate > 4:
            ...
            jterate = 1
    fig2.tight_layout()
    fig2.savefig(f"Images/generation{sys.argv[-1]}.png", bbox_inches='tight', dpi=200)  
    logger.info(f" saved to Images/generation{sys.argv[-1]}.png")
...
def plot_compare(model, images=None): 
    ...
    for i in range(n_to_show):
        ...
        if iterate > 4:
            ...
            jterate = 1
    fig2.tight_layout()
    fig2.savefig(f"Images/reconstruction{sys.argv[-1]}.png", bbox_inches='tight', dpi=200)
    logger.info(f" saved to Images/generation{sys.argv[-1]}.png")
