...
def vae_generate_images(vae,Z_DIM,n_to_show=10):
    ...
    reconst_images1 = foo.roll_X(reconst_images[...,1], roll_axis='lat', roll_steps=-64) # remove extra fields 
    reconst_images0 = foo.roll_X(reconst_images[...,2], roll_axis='lat', roll_steps=-64) # remove extra fields 
    ...
    for i in range(n_to_show):
        ...
        ef.geo_contourf(m, ax[iterate], 0, *cplt.Greenwich(LON, LAT,img0),levels, "seismic", f" generated zg500", put_colorbar=True)
...
def plot_compare(model, images=None): # Plot images generated by the autoencoder
    ...
    reconst_images1 = foo.roll_X(reconst_images[...,1], roll_axis='lat', roll_steps=-64) # remove extra fields
    reconst_images0 = foo.roll_X(reconst_images[...,2], roll_axis='lat', roll_steps=-64) #reconst_images[...,0] # remove extra fields
    print("reconst_images.shape = ",reconst_images.shape)
    images1 = foo.roll_X(images[...,1], roll_axis='lat', roll_steps=-64)
    images0 = foo.roll_X(images[...,2], roll_axis='lat', roll_steps=-64)
    ...
    for i in range(n_to_show):
        ...
        m.set_extent([-180,180, 30, 90], crs=data_proj)
        if jterate == 0:
            ef.geo_contourf(m, ax[iterate], 0, *cplt.Greenwich(LON, LAT,img0),levels, "seismic", f"actual zg500", put_colorbar=True)
            ef.geo_contour (m, ax[iterate], 0, *cplt.Greenwich(LON, LAT,img1),levels, "PuRd", "summer")
        else:
            ef.geo_contourf(m, ax[iterate], 0, *cplt.Greenwich(LON, LAT,img0),levels, "seismic", f"reconstructed zg500", put_colorbar=True)
            ef.geo_contour (m, ax[iterate], 0, *cplt.Greenwich(LON, LAT,img1),levels, "PuRd", "summer")
        iterate += 1
        if iterate > 4:
            iterate = 0
