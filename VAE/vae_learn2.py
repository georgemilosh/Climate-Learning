...
def run_vae(checkpoint_name, myinput='N', SET_YEARS=range(100)):
    '''
    ...
    return X, lat, lon, vae, N_EPOCHS, INITIAL_EPOCH, checkpoint_path, history, history_loss
...
if __name__ == '__main__': # we do this so that we can then load this file as a module in reconstruction.py
    ...
    X, lon, lat, vae, N_EPOCHS, INITIAL_EPOCH, checkpoint_path, history, history_loss = run_vae(checkpoint_name=checkpoint_name,myinput=myinput)
