...
# This routine is written for one parameter: input folder for VAE weights. It computes the analogs using the distance based on local temperature, soil moisture and North Atlantic geopotential without autoencoder
# The new usage analogue.py <folder> <coefficients>
#   example <coefficients> = [1,10,100]
import os, sys
import shutil
import json
import pickle
...
def classify(fold_folder, evaluate_epoch, vae, X_tr, z_tr, Y_tr, X_va, z_va, Y_va, u=1): # Overwriting the functionality so that it saves analogues
    ...
    ind_new_va = {}
    ind_va = {}
    for checkpoint in [1]: # This way we maintain the datastructure found in files generated by analogue.py
