    ...
    for checkpoint in checkpoints:
        ...
        while found == 0:
            ...
            if not os.path.exists(checkpoint_path_check): # it could be that the training was unstable and we have to look for a checkpoint just before:
                ...
                open_file = open(f'{fold_folder}/analogues.pkl', "rb")
                analogues = pickle.load(open_file)
                open_file.close()
                ind_new_tr = analogues['ind_new_tr'][checkpoint]
                ind_new_va = analogues['ind_new_va'][checkpoint]
                logger.info(f"{ind_new_tr.shape = }, {ind_new_va.shape = }")
                #dist_tr = analogues['dist_tr'][checkpoint]
                #dist_va = analogues['dist_va'][checkpoint]
                logger.info(f"{X_tr[ind_new_tr[0,:nn]] = }")
